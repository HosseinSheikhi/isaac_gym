...
class KinovaCameraIKEnv(BaseTask):
    ...
    def load_assets(self):
        ...
        for asset_desc in asset_descriptors:
            #for override in [True, False]:
                ...
                self.assets.append(self.gym.load_asset(self.sim, asset_root, asset_file, asset_options))
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        for i in range(self.num_envs):
            ...
            pose.p = gymapi.Vec3(*get_axis_params(0.25, self.up_axis_idx))
            object_actor = self.gym.create_actor(env_ptr, self.assets[np.random.randint(len(self.assets))], pose, "random_asset"+str(i), i,2)
    ...
    def compute_observations(self):
        ...
        self.goal_obs[(self.reset_complete==1).nonzero(as_tuple=False).squeeze(-1)] = self.sensors_tensors[(self.reset_complete==1).nonzero(as_tuple=False).squeeze(-1)] # self.reset_complete==1 means already is reached to target but not hommed yet
        self.current_obs[(self.reset_complete==2).nonzero(as_tuple=False).squeeze(-1)] = self.sensors_tensors[(self.reset_complete==2).nonzero(as_tuple=False).squeeze(-1)]
        
        #torch.stack(self.sensors_tensors,  out=self.obs_buf)
        torch.cat((self.goal_obs, self.current_obs), 0 , out=self.obs_buf)
        if frame_no < 10000 and frame_no%50==0:
            #for i in range(self.num_envs):
                # write tensor to image
            i=0
            fname = os.path.join(self.img_dir, "sensor-%04d-%04d.png" % (frame_no, i))
            sensor_img = self.obs_buf[i].cpu().numpy()
    ...
    def reach_target(self, env_ids):
        ...
        """
        pos = tensor_clamp(
            self.kinova_home.unsqueeze(0) + 2.0 * (torch.rand((len(env_ids), self.num_kinova_dofs), device=self.device) - 0.1),
            self.kinova_dof_lower_limits, self.kinova_dof_upper_limits)
        self.kinova_dof_pos[env_ids, :] = pos
        self.kinova_dof_vel[env_ids, :] = torch.zeros_like(self.kinova_dof_vel[env_ids])
        self.kinova_target_dof_pos[env_ids, :self.num_kinova_dofs] = pos
    ...
    def reset(self, env_ids):
        """
            reset kinova (or any other actor): go to a random position in a vaccinity of the home pose with 0 veliocities
        """
        ...
        # todo
        #self.obs_buf[env_ids] = torch.zeros((self.num_envs, *self.num_obs))
        #self.states_buf[env_ids] = torch.zeros((self.num_envs, self.num_states))
