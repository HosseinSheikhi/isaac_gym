...
class VecTaskPython(VecTask):
    ...
    def step(self, actions):
        ...
        if self.task.obs_buf.dim()==1:
            ...
            return self.task.obs_buf.to(self.rl_device), self.task.rew_buf.to(self.rl_device), self.task.reset_buf.to(self.rl_device), self.task.valid, self.task.extras
