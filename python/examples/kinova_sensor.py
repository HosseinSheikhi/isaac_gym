...
# Load franka asset
asset_root = "../../assets"
franka_asset_file = "urdf/kinova_gen3/urdf/GEN3_URDF_V12.urdf"
...
envs = []
franka_handles = []
franka_hand = "Bracelet_Link" #"Bracelet_Link"
...
pose.r = gymapi.Quat(0.7, 0.0, 0.0, -0.7)
...
for i in range(num_envs):
    ...
    attractor_properties.target.p.y -= 0.5
    attractor_properties.target.p.z += 0.4
...
for i in range(num_envs):
    ...
    body_handle = gym.find_actor_rigid_body_handle(envs[i], actor_handle, "EndEffector_Link")
...
def update_franka(t):
    ...
    for i in range(num_envs):
        ...
        pose.p.y = 0.4 + 0.1 * math.cos(2.5 * t - math.pi * float(i) / num_envs)
        pose.p.z =0.2+  0.2 * math.cos(1.5 * t - math.pi * float(i) / num_envs)
...
if not os.path.exists("kinova_images"):
    os.mkdir("kinova_images")
frame_count = 0
...
while not gym.query_viewer_has_closed(viewer):
    ...
    if np.mod(frame_count, 300) == 0:
        for i in range(num_envs):
            for j in range(2):
                # The gym utility to write images to disk is recommended only for RGB images.
                rgb_filename = "kinova_images/rgb_env%d_cam%d_frame%d.png" % (i, j, frame_count)
                ...
                #normalized_depth_image.show(title="image")
                normalized_depth_image.save("kinova_images/depth_env%d_cam%d_frame%d.jpg" % (i, j, frame_count))
